...
public class PartyService {
    ...
    // ★ 5. 비활성 거래처 목록 조회 기능
    public List<PartyDTO> getAllDeactivatedParties() throws SQLException {
        // DAO에 selectDeactivated() 메서드가 있다고 가정
        return dao.selectDeactivated();
    }
    ...
    public void deleteParty(String partyId) throws SQLException {
    ...
    //  6. 거래처 복구 (Restore) 기능
    public void restoreParty(String partyId) throws SQLException {
        if (partyId == null || partyId.trim().isEmpty()) {
            throw new IllegalArgumentException("거래처 ID는 필수입니다.");
        }

        // DAO의 restoreById 호출
        int result = dao.restoreById(partyId);

        if (result == 0) {
            throw new IllegalArgumentException("거래처 복구 실패: 해당 ID를 찾을 수 없습니다.");
        }
    }
